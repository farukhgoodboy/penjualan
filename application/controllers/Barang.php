			...
			if($_POST)
			{
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					foreach($_POST['kode'] as $k)
					{
						...
						$insert = $this->m_barang->tambah_baru($kode, $nama, $id_kategori_barang, $size ,  $stok, $harga, $keterangan);
						if($insert){
							$inserted++;
						}
						$no_array++;
					...
					if($inserted > 0)
					{
						echo json_encode(array(
							'status' => 1,
							'pesan' => "<i class='fa fa-check' style='color:green;'></i> Data barang berhasil disimpan."
						));
					}
					else
					{
				...
				if($this->input->is_ajax_request())
				{
					...
					if($_POST)
					{
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							$update = $this->m_barang->update_barang($id_barang, $kode_barang, $nama,  $id_kategori_barang, $size,  $stok, $harga, $keterangan);
							if($update)
							{
								echo json_encode(array(
									'status' => 1,
